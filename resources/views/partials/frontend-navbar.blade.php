<div x-data="{ darkMode: false }" :class="{ 'dark': darkMode }" class="bg-white dark:bg-gray-900">
    <div class="container mx-auto px-4">
        <div class="flex items-center justify-between py-3 border-b flex-wrap">
            ...
            @if (!request()->routeIs('articles.search'))
                <div x-data="{
                    open: false,
                    query: '{{ request('q') }}',
                    recent: ['Berita Sumba', 'Politik', 'Olahraga'],
                    filtered: [],
                    init() { this.open = false; }
                }" class="relative flex-1 justify-center px-2 mt-3 md:mt-0 items-center">
                    <!-- Mobile Icon -->
                    ...
                    <form action="{{ route('articles.search') }}" method="GET"
                        class="hidden md:flex relative w-40 md:w-64 transition-all duration-300 ease-in-out focus-within:w-96">
                        <div
                    ...
                    <div x-show="open" x-transition x-cloak
                        ...
                        <div class="w-full max-w-md relative">
                            ...
                            <!-- Mobile input -->
                            <form action="{{ route('articles.search') }}" method="GET" class="w-full relative">
            ...
            <div class="flex items-center space-x-2 mt-3 md:mt-0 flex-shrink-0">
                ...
                <button @click="darkMode = !darkMode"
                    class="bg-gray-200 dark:bg-gray-700 text-gray-600 dark:text-gray-300 px-3 py-2 rounded-full hover:bg-gray-300 dark:hover:bg-gray-600 transition">
        ...
        <nav class="py-3 border-b bg-white dark:bg-gray-900">
            <div class="flex overflow-x-auto scrollbar-hide space-x-4 px-2 whitespace-nowrap w-full">
                <a href="{{ url('/') }}"
                    class="px-2 py-2 flex-shrink-0 {{ request()->is('/') ? 'text-red-600 border-b-2 border-red-600' : 'text-gray-700 dark:text-gray-300 hover:text-red-600' }}">
                    Home
                </a>
                @foreach ($categories as $category)
                    <a href="{{ route('articles.category', $category->slug) }}"
                        class="px-2 py-2 flex-shrink-0 {{ $currentSlug === $category->slug ? 'text-red-600 border-b-2 border-red-600' : 'text-gray-700 dark:text-gray-300 hover:text-red-600' }}">
                        {{ $category->name }}
                    </a>
                @endforeach
            </div>
