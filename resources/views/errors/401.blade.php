...
@section('content')
    <div class="min-h-[80vh] flex flex-col items-center justify-center text-center px-4">
        ...
        {{-- Ilustrasi Unauthorized --}}
        <div class="w-64 md:w-96 mb-6 relative">
            <img src="{{ asset('images/401-lock.svg') }}" alt="401 Unauthorized" class="w-full animate-bounce-slow">
            <span class="absolute top-2 left-2 text-sm text-gray-500 dark:text-gray-400">Oops!</span>
        </div>

        {{-- Judul --}}
        <h1 class="text-5xl font-extrabold text-yellow-600 mb-2">401</h1>
        ...
        <div class="flex flex-col sm:flex-row gap-4 mb-8">
            <a href="{{ url('/') }}"
                class="bg-yellow-600 hover:bg-yellow-700 text-white font-semibold px-6 py-3 rounded shadow transition">
                Kembali ke Beranda
            </a>
            <a href="{{ route('login') }}"
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-6">
                    @foreach ($trending as $article)
                        <a href="{{ route('articles.show', $article->slug) }}"
                            ...
                            <div class="p-3">
                                <h3 class="font-medium text-sm line-clamp-2 group-hover:text-yellow-600 transition">
                                    {{ $article->title }}
                                </h3>
                            </div>
                        </a>
